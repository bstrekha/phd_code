...
import numpy as np
from get_TM_dipole_circular_grating_LDOS_boundW_local_constraints import get_TM_dipole_circular_grating_LDOS_global_bound_local_constraints
import os.path
import argparse
...
parser.add_argument('-subreg_per_lambda',action='store',type=float,default=2.0)
...
dw_list = np.logspace(-7, 1, 320) #in units of w0 
...
for k in range(len(dw_list)):
    ...
    numSubRegions = round(args.subreg_per_lambda*round(outer_R))
    ans = list(map(float,get_TM_dipole_circular_grating_LDOS_global_bound_local_constraints(cur_chi, 1.0, 0.0, outer_R, 0.5, 1.5, 600, Qabs=Qsrc, numSubRegions=numSubRegions)))
