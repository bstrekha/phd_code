...
def get_TM_dipole_circular_grating_LDOS_global_bound(chi, wvlgth, r_inner, r_outer, pml_sep, pml_thick, gpr, Qabs=np.inf, justAsym=False, opttol=1e-2, fakeSratio=1e-2, iter_period=20):
    ...
    des_mask = np.zeros(r_gridnum, dtype=np.bool)
    ...
    SymUM = (UM + UM.conj().T) / 2
    AsymUM = (UM - UM.conj().T) / 2j

    Id = sp.eye(Ginv.shape[0], format='csc')
    ...
    O_lin = 0.5 * (1j/2) * np.conj(omega) * (-Ginv.conj().T @ np.conj(S1)) #ATTENTION
    ...
    enh = optdual / (2*np.pi/wvlgth/8) #calculate enhancement factor of scattered LDOS relative to single frequency vacuum ldos which in the 2D TM case is omega/8
