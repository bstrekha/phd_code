...
from objective_TM_LDOS_PB_Wfun import designdof_ldos_objective
...
for design_i in range(len(design_vals)):
    ...
    for Qabs in Qabslist:
        ...
        optfunc = lambda dof, grad: designdof_ldos_objective(dof, grad, epsval, design_mask, dl, source, omega0, Qabs, epsVac, Npml, opt_data)
        ...
        plt.figure()
        plt.imshow(np.reshape(opt_design[Npml+Npmlsep+Distx:Npml+Npmlsep+Distx+Mx,Npml+Npmlsep:Npml+Npmlsep+My], (Mx,My)),cmap='Greys')
