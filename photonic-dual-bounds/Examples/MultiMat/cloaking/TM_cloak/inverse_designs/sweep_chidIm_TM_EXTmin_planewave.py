...
for design_i in range(1):
    ...
    for chidIm in chidIm_list:
        #for chidIm sweeps, start over dof each run for large chidIm values
        if chidIm >= 1e-2:
            ndof = np.sum(design_mask)
            if args.init_type=='vac':
                designdof = np.zeros(ndof)
            ...
            if args.init_type=='rand':
                designdof = np.random.rand(ndof)
        #update to new chid
        print('at chidIm', chidIm)
        chid = args.chidRe + 1j*chidIm
        opt_data['count'] = 0 #refresh the iteration count
        
        opt_data['name'] = args.name + f'_chidIm{chidIm:.8e}'
        ...
        print(f'chidIm{chidIm:.8e} best Pext and enhancement found via topology optimization', min_ext, min_enh)
        ...
        print(f'chidIm{chidIm:.8e} binarized opt result Pext and enhancement found via topology optimization', bin_obj, bin_obj/background_term)
        ...
        np.save(args.name+'_chidIm', np.array(chidIm_list[0:len(inv_des_list)]))