...
import nlopt
from objective_TM_ext import designdof_ext_objective
...
R_list = args.des_param1*np.geomspace(1.0, args.maxratioRouterRinner, args.numRsweep+1)
R_list = R_list[1:] #used when using np.geomspace(1.0, args.maxratioRouterRinner, args.numRsweep)
inv_des_list = []

for outeri in range(len(R_list)):
    Router = R_list[outeri]
    des_params = [args.des_param1, Router]
    design_x = 2*R_list[-1]
    design_y = 2*R_list[-1]
    dl = 1.0/args.gpr
    ...
    #background[Npmlsep:Npmlsep+Mx//3, Npmlsep:Npmlsep+My//3] =  0
    if des_region == 'rect':
    ...
    np.save(opt_data['name'] + '_Router' + str(Router)+ '_backgrounddof', background_mask[Npmlsep:Npmlsep+Mx, Npmlsep:Npmlsep+My]*1.0)
    ...
    if outeri <= 1:
        #for first iteration, start from scratch
        if args.init_type=='vac':
            designdof = np.zeros(ndof)
        if args.init_type=='ones':
            designdof = np.ones(ndof)
        ...
        #for larger R values after first, use design from smaller previous run
        prevdof = np.zeros_like(designdof)
        prevdof[:] = designdof[:]
        designdof = np.zeros(ndof)
        designdof[:] = fulldesigndof[design_mask]
        
    #update to new chid
    print('at Router', Router)
    opt_data['count'] = 0 #refresh the iteration count

    opt_data['name'] = args.name
    ...
    print('background_term: ', background_term, flush=True)

    optfun = lambda dof, dofgrad: designdof_ext_objective(dof, dofgrad, chif, background_mask, chid, design_mask, Npml, dl, cx, omega0, Qabs, opt_data)

    test_background_term = ext_objective(np.zeros(ndof), chif, background_mask, chid, design_mask, Npml, dl, cx, omega0, Qabs)
    print('optfun test (equal to background_term?): ', test_background_term, flush=True)
    print('|background_term2 - background_term|/|background_term|', abs(test_background_term - background_term)/abs(background_term))
    ...
    print(f'Router{Router:.4e} best Pext and enhancement found via topology optimization', min_ext, min_enh)
    #print the binarized performance
    bin_obj = ext_objective(np.round(designdof), chif, background_mask, chid, design_mask, Npml, dl, cx, omega0, Qabs)
    print(f'Router{Router:.4e} binarized opt result Pext and enhancement found via topology optimization', bin_obj, bin_obj/background_term)

    #save final opt design
    fulldesigndof = np.zeros_like(design_mask) * 1.0
    fulldesigndof[design_mask] = designdof[:]
    #prev_design_mask = design_mask.copy()
    np.save(opt_data['name'] + '_Router' + str(Router)+ '_optdof', fulldesigndof[Npmlsep:Npmlsep+Mx, Npmlsep:Npmlsep+My])
    if args.init_file != '':
    ...
    np.save(args.name+'_Router', np.array(R_list[0:len(inv_des_list)]))