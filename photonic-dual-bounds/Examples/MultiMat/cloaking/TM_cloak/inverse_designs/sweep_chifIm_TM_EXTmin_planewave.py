...
parser.add_argument('-chifIm_start',action='store',type=float,default=1.0)
parser.add_argument('-chifIm_end',action='store',type=float,default=0.0001)
...
#         designdof = np.random.rand(ndof)
    ...
    for chifIm in chifIm_list:
        #reset dof when doing chi sweeps
        ndof = np.sum(design_mask)
        if args.init_type=='vac':
            designdof = np.zeros(ndof)
        if args.init_type=='ones':
        ...
        if args.init_type=='rand':
            designdof = np.random.rand(ndof)
        #update to new chif
        print('at chifIm', chifIm)
        chif = args.chifRe + 1j*chifIm
        ...
        opt_data['name'] = args.name + f'_chifIm{chifIm:.8e}'
        ...
        print(f'chifIm{chifIm:.8e} best Pext and enhancement found via topology optimization', min_ext, min_enh)
        ...
        print(f'chifIm{chifIm:.8e} binarized opt result Pext and enhancement found via topology optimization', bin_obj, bin_obj/background_term)
        ...
        np.save(args.name+'_chifIm', np.array(chifIm_list[0:len(inv_des_list)]))