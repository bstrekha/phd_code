...
def sendtoHPC(ram, time, chifRe, chifIm, chidRe, chidIm, wavelength, des_region, design_x, design_y, 
    ...
    command = f'bash sp_all_parallel_cloak_bound_1side.sh -p {ram} -t {time} -r {chifRe} -i {chifIm} -R {chidRe} -I {chidIm} -w {wavelength} -e {des_region} -x {design_x} -y {design_y} -D {des_params[0]} -E {des_params[1]} -g {gpr} -o {obj} -s {save} -O {opttol} -Q {Qabs} -F {fakeSratio} -T {iter_period} -m {pml_sep} -M {pml_thick} -c {do_checks} -a {nprojx} -b {nprojy}'
...
iter_period=110
do_checks=False
des_params=[des_param0, des_param1]
...
#         for Qabs in np.flip(np.logspace(2, 7, 31))/2.0:
            sendtoHPC(mem_mbs, run_time_hrs, np.real(chif), np.imag(chif), np.real(chid), np.imag(chid), wavelength, 'rect', design_x, design_y, nprojx, nprojy, des_params, gpr, 'EXT', save, opttol, Qabs, fakeSratio, iter_period, pml_sep, pml_thick, do_checks)