...
def BFGS_fakeS_with_restart(initdof, dfunc, dgfunc, validityfunc, mineigfunc, gradConverge=False, opttol=1e-2, fakeSratio=1e-2, reductFactor=0.1, iter_period=20, min_iter=6):
    ...
    while True: #gradual reduction of modified source amplitude, outer loop
        ...
        dualval, grad = dgfunc(dof, jnp.array(fSlist))
        ...
        while True:
            ...
            Dopt = np.inf
            while True:
                tmp_dual = dfunc(dof+alpha*pdir, fSlist)
                if tmp_dual < Dopt: #the dual is still decreasing as we backtrack, continue
                    Dopt = tmp_dual; alphaopt=alpha
                else:
                    alphaopt=alpha ###ISSUE!!!! #TODO(alessio): Ask original author why this is an issue and fix it
                    break
            ...
            tmp_dual, tmp_grad = dgfunc(dof+delta, fSlist)
