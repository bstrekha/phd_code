...
def fakeS_with_restart_singlematrix(initdof, dgHfunc, validityfunc, mineigfunc, opttol=1e-2, gradConverge=False, fakeSratio=1e-2, reductFactor=0.1, iter_period=20, min_iter=6, verbose=False):
    """
    ...
    # Interruption (e.g. ^C) and termination (e.g. slurm sending SIGTERM) handling
    # Many runs spend a long time very close to the optimum (but need very low tolerance to get there in the first place)
    # Rather than tuning the tolerance, we can just catch the signal and return the current values
    def int_handler(signum, frame):
        print("SIGINT detected, exiting.")
        raise KeyboardInterrupt

    def term_handler(signum, frame):
        print("SIGTERM detected, exiting.")
        raise KeyboardInterrupt
    ...
    try:
        while True: #outer loop; gradually reduce amplitudes of fake sources
            ...
            while True:
                ...
                if verbose: print('normgrad is', normgrad)
                ...
                if iternum % iter_period == 0:
                    if verbose: print('previous dual is', prevD)
                ...
                if doGD:
                    if verbose: print('do regular gradient step')
                ...
                if verbose: print('alpha before feasibility backtrack', alpha)
                while validityfunc(dof+alpha*pdir)<=0:
                    alpha *= c2
            
                alpha_feas = alpha
                if verbose: print('alpha before backtracking is', alpha_feas)
                alphaopt = alpha
                Dopt = np.inf
                while True:
                ...
                if verbose: print('stepsize alphaopt is', alphaopt, '\n')
                dof += alphaopt*pdir
                if doGD:
        ...
        ########END OF OUTER LOOP###############
        return dof, dofgrad, dualval, objval, 1 # 1 is for converged flag 
    except KeyboardInterrupt as e:
        print('Exception caught. Calculating final values without fake sources:', flush=True)
        dualval = dgHfunc(dof, dofgrad, dofHess, [], get_grad=True, get_Hess=(not doGD)) 
        objval = dualval - dof @ dofgrad
        return dof, dofgrad, dualval, objval, 0
...
def fakeS_single_restart_Newton(initdof, dgHfunc, validityfunc, mineigfunc, fSlist=[], opttol=1e-2, gradConverge=True, singlefS=True, fakeSratio=1e-2, iter_period=20, min_iter=6):
    ...
    while True: #outer loop; gradually reduce amplitudes of fake sources
        ...
        while True:
            ...
            if printperiod % iternum == 0: print('mineig of ZTT is', mineigw)
            ...
            if printperiod % iternum == 0: print('alpha before feasibility backtrack', alpha)
            while validityfunc(dof+alpha*pdir)<=0:
                alpha *= c2
        
            alpha_feas = alpha
            if printperiod % iternum == 0: print('alpha before backtracking is', alpha_feas)
            alphaopt = alpha
            Dopt = np.inf
            while True:
                tmp = dualfunc(dof+alpha*pdir)
            ...
            if printperiod % iternum == 0: print('stepsize alphaopt is', alphaopt, '\n')
            dof += alphaopt*pdir
            if doGD:
                alphaopt_grad = alpha_newstart
            else:
