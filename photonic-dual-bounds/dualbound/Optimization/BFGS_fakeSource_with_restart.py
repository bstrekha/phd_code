...
def BFGS_fakeS_with_restart(initdof, dgfunc, validityfunc, mineigfunc, gradConverge=False, opttol=1e-2, fakeSratio=1e-2, reductFactor=0.1, iter_period=20, min_iter=6, verbose=False):
    ...
    try:
        while True: #gradual reduction of modified source amplitude, outer loop
            ...
            while True:
                ...
                if verbose: print('pdir dot grad is', p_dot_grad)
                c_reduct = 0.7; c_A = 1e-4; c_W = 0.9
                alpha = alpha_start = alpha_last
                if verbose: print('starting alpha', alpha_start)
                ...
                if added_fakeS:
                    ...
                    if verbose: print('updating Hinv')
                    gamma = tmp_grad - grad
                ...
                if verbose: print('normgrad is', la.norm(grad))
                ...
                if iternum % iter_period==0:
                    if verbose: print('prev_dualval is', prev_dualval)
        ...
        return dof, grad, dualval, objval, 1
    
    except KeyboardInterrupt as e:
        print('Exception caught. Calculating final values without fake sources:', flush=True)
        dualval = dgfunc(dof, grad, [], get_grad=True)
        return dof, grad, dualval, objval, 0
