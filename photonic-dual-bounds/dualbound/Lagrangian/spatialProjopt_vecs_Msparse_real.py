...
# -*- coding: utf-8 -*-
"""
Created on Mon May 16 12:37:35 2022
...
def get_ZTTcho_Tvec_gradTvec_real(ZTT, gradZTT, ZTS_S, gradZTS_S, chofac=None):
    if chofac is None:
        ZTTcho = chol.cholesky(ZTT)
    else:
        ZTTcho = chofac.cholesky(ZTT)
    ...
    gradTvec = []
    for i in range(len(gradZTT)):
        gradTvec.append(ZTTcho.solve_A(-(gradZTT[i] + gradZTT[i].T) @ Tvec + gradZTS_S[i]))
