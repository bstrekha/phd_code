...
def get_TE_dipole_field(wvlgth, Nx, Ny, Npmlx, Npmly, dx, dy, cx,cy, pol, amp=1.0, Qabs=np.inf, chigrid=None):
    '''
    amp if you want a dipole, you need to pass 1/dl/dl
    Solve the full Maxwell's equations 
    
    '''
    assert pol == 1 or pol == 2, 'polarization is in x (1) or in y (2)'
...
def get_TE_linesource_field(wvlgth, Nx, Ny, Npmlx, Npmly, dx, dy, cx, pol, amp=1.0, Qabs=np.inf, chigrid=None):
    assert pol == 1 or pol == 2, 'polarization is in x (1) or in y (2)'
    omega = 2*np.pi/wvlgth * (1 + 1j/2/Qabs)
    A = build_TE_vac_A(omega, Nx, Ny, Npmlx, Npmly, dx, dy)
    A = A.tocsr()
...
if __name__ == '__main__':
    design_mask = np.zeros((100,100), dtype=bool)
    design_mask[30:50,30:50] = True
    G = get_Yee_TE_masked_GreenFcn(1, 100, 100, design_mask, 10, 10, 1/200, 1/200)
    print(np.max(np.imag(G)))