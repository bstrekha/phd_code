...
def get_diagM_from_chigrid(omega, chigrid):
    Nx,Ny = chigrid.shape
    return -sp.diags(chigrid.flatten() * omega**2, offsets=0)

def get_TM_dipole_field(wvlgth, dL, Nx, Ny, cx, cy, Npml, bloch_x=0.0, bloch_y=0.0, amp=1.0, Qabs=np.inf, chigrid=None):
    """
    ...
    '''
    curl curl E - k^2 E = i omega 1/dl/dl where 1/dl/dl is a representation of the delta function
    And the curl reduces to a scalar laplacian 
    Nx does NOT include the PML region 
    '''
    shape = (Nx,Ny)
    omega = (2*np.pi*C_0/wvlgth) * (1 + 1j/2/Qabs)
...
def get_TM_linesource_field(wvlgth, dL, Nx, Ny, cx, Npml, bloch_x=0.0, bloch_y=0.0, amp=1.0, Qabs=np.inf, chigrid=None):
    shape = (Nx,Ny)
    omega = (2*np.pi*C_0/wvlgth) * (1 + 1j/2/Qabs)
    ...
    sourcegrid = np.zeros((Nx,Ny), dtype=complex)
    # For a line source, delta function is only in x, i.e. delta(x-x0) and not delta(x - x0) delta(y - y0) so the amplitude is 1/dl
    sourcegrid[cx,:]= amp / (dL)
...
def get_TM_linesource_field_cy(wvlgth, dL, Nx, Ny, cy, Npml, bloch_x=0.0, bloch_y=0.0, amp=1.0, Qabs=np.inf, chigrid=None, angle=0.0):
    shape = (Nx,Ny)
    omega = (2*np.pi*C_0/wvlgth) * (1 + 1j/2/Qabs)
    ...
    sourcegrid = np.zeros((Nx,Ny), dtype=complex)
    # For a line source, delta function is only in x, i.e. delta(x-x0) and not delta(x - x0) delta(y - y0) so the amplitude is 1/dl
    sourcegrid[:,cy]= amp / (dL)
    for i in range(len(sourcegrid[:,cy])):
                   sourcegrid[i, cy] *= np.exp(1j*2*np.pi/wvlgth*(i*np.sin(np.radians(angle)))*dL)
...
def get_TM_linesource_field_cx_angle(wvlgth, dL, Nx, Ny, cx, angle, Npml, bloch_x=0.0, bloch_y=0.0, amp=1.0, Qabs=np.inf, chigrid=None):
    #angle in degrees
    shape = (Nx,Ny)
    omega = (2*np.pi*C_0/wvlgth) * (1 + 1j/2/Qabs)
    ...
    # For a line source, delta function is only in x, i.e. delta(x-x0) and not delta(x - x0) delta(y - y0) so the amplitude is 1/dl
    sourcegrid[cx,:]= amp / (dL)
    for i in range(len(sourcegrid[cx,:])):
                   sourcegrid[cx, i] *= np.exp(1j*2*np.pi/wvlgth*(i*np.sin(np.radians(angle)) + cx*np.cos(np.radians(angle)))*dL)
    RHS = 1j*omega*sourcegrid.flatten()
...
def get_TM_field(wvlgth, dL, Nx, Ny, Npml, sourcegrid, chigrid):
    shape = (Nx,Ny)
    omega = (2*np.pi*C_0/wvlgth) * (1 + 1j/2/np.inf)    
    M = get_TM_MaxwellOp(wvlgth, dL, Nx,Ny, Npml, bloch_x=0, bloch_y=0, Qabs=np.inf)
    if not (chigrid is None):
        M += get_diagM_from_chigrid(omega, chigrid)
    
    RHS = 1j*omega*sourcegrid.flatten()
    return np.reshape(spla.spsolve(M, RHS), shape)
...
def get_Gddinv(wvlgth, dL, Nx, Ny, Npml, designMask, bloch_x=0.0, bloch_y=0.0, Qabs=np.inf, chigrid=None):
...
if __name__ == '__main__':
    ...
    sourcegrid = np.zeros((Nx, Ny))
    rr1, cc1 = disk((40//2, 40//2), 5, shape=sourcegrid.shape)
    sourcegrid[rr1, cc1] = 1

    chigrid = np.zeros((Nx, Ny))
    rr2, cc2 = disk((40//2, 40//2), 10, shape=chigrid.shape)
    chigrid[rr2, cc2] = 10
    rr2, cc2 = disk((40//2, 40//2), 6, shape=chigrid.shape)
    chigrid[rr2, cc2] = 0

    plt.imshow(sourcegrid)
    plt.colorbar()
    plt.savefig('sourcegrid')
    plt.clf() 

    plt.imshow(np.real(chigrid))
    plt.colorbar()
    plt.savefig('chigrid')
    plt.clf()

    Ez = get_TM_field(wv, dl, Nx, Ny, Npml, sourcegrid, chigrid)
